...
function MapView() {
  ...
  const ors = new OpenRouteService.Directions({ api_key: '5b3ce3597851110001cf6248af861090d6d647db843e8a737cc997f7' });
  ...
  const fetchRoute = (start, end) => {
    ...
    .then(response => {
      const coords = response.features[0].geometry.coordinates.map(coord => [coord[1], coord[0]]);
      const distanceInMeters = response.features[0].properties.summary.distance;
      const durationInSeconds = response.features[0].properties.summary.duration;
